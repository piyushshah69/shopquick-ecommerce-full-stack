...
// Helper functin to get a cart by user ID or guest ID
  ...
  try {
    ...
    if (cart) {
      ...
      if (productIdx > -1) {
        ...
        cart.products[productIdx].quantity += Number(quantity);
      } else {
        cart.products.push({
          productId,
          name: product.name,
    ...
    if (guestCart) {
      ...
      if (userCart) {
        ...
        guestCart.products.forEach((guestItem) => {
          const productIdx = userCart.products.findIndex((item) => item.productId.toString() === guestItem.productId.toString() && item.size === guestItem.size && item.color === guestItem.color);
          if (productIdx > -1) {
            // If items exits in user update the quantity
        ...
        // If the user has no existing cart assign the cart to user
